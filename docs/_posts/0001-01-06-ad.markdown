...
Importantly, the construction of a derivative function in this way is something that can be done by a computer operating on the source code representation.
...
However, we are not going to use forward-mode AD here, because we know that we will be using reverse-mode when we perform inverse rendering, and we want to check that our reverse-mode AD is working properly.
So, can we still compute this gradient image with reverse-mode AD?
The answer is yes, _but_, it must be done one pixel at a time.
